...
*
*/
utils.lottery={type:"ssq",red:[],blue:[],zhu:0};     //utils.lottery存放用户投注方案
		 ...
		 if(utils.hasCls(target,"active")){
			 ...
			 if(utils.lottery[type].length>0){
				...
				if(type=="red"&&utils.lottery[type].length<35){
				   ...
				   }else if(type=="blue"&&utils.lottery[type].length<16){
					   utils.lottery[type].push(value); 
					   }
				}
...
utils.addEvt(window,'devicemotion',function(eventData){
   ...
   var speed=Math.abs(x + y + z - last_x - last_y - last_z) / diffTime * 10000; 
	...
	var results=[];
	for(var i=0;i<5;i++){
		results.push({"red":utils.randoms({min:1,max:35,many:6}),"blue":utils.randoms({min:1,max:16,many:1})});//随机摇号结果集
		}
	var count=0;
	...
	var timer=setInterval(function(){
		  if(count==results.length){
			  clearInterval(timer);
		    	...
		    	utils.initlottery();
				for(i=0;i<results[count].red.length;i++){
					utils.addCls(lidoms[results[count].red[i]-1].parentNode,'active');
					utils.lottery.red.push(lidoms[results[count].red[i]-1].innerHTML);
					}
				 utils.addCls(lidoms[results[count].blue[0]+34].parentNode,'active');  
				 utils.lottery.blue.push(lidoms[results[count].blue[0]+34].innerHTML);
		         var plan=utils.calZhu(utils.lottery);
				 if(plan>=0){
				  utils.$(".betNum")[0].innerHTML=plan;
				  utils.$(".betMoney")[0].innerHTML=2*plan;
