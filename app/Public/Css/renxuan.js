window.onload=function(){
	...
	var type_=utils.$(".clearfix li")[0].parentNode.id;
	utils.type=parseInt(type_[type_.length-1]);
	utils.lottery={type:"ren"+utils.type,ball:[],zhu:0};     //utils.lottery存放用户投注方案
	...
	utils.addEvt(window,'devicemotion',function(eventData){
	   ...
	   var speed=Math.abs(x + y + z - last_x - last_y - last_z) / diffTime * 10000; 
		...
		var results=[];
		for(var i=0;i<5;i++){
			results.push(utils.randoms({min:1,max:11,many:utils.type}));//随机摇号结果集
			}
		var count=0;
