...
var popUpCal = {
	...
	monthNames : [ '一月', '二月', '三月', '四月', '五月', '六月','七月', '八月', '九月', '十月', '十一月', '十二月' ], // Names
	dateFormat :'YMD-',
	...
	speed :1, // Speed of display/closure
	autoPopUp :'focus', // 'focus' for popup on focus, 'button' for trigger
	closeAtTop : '', // True to have the clear/close at the top, false to
	...
	formatDate : function(day, month, year, hour, minute) {
		...
		for ( var i = 0; i < 3; i++) {
			dateString += this.dateFormat.charAt(3)
							...
							+ day : (this.dateFormat.charAt(i) == 'M' ? (month < 10 ? '0' : '')
							+ month : (this.dateFormat.charAt(i) == 'Y' ? year : '?')));
		}
		if (this.timeSeparators && this.timeSeparators.length > 1 && this.timeSeparators[0] && hour != null && minute != null) {
			dateString += this.timeSeparators[0]
							+ ((this.hourString(hour) >= 10) ? hour : ('0' + hour))
	...
	connectCalendar : function(target) {
		...
		if (this.autoPopUp == 'button' || this.autoPopUp == 'both') { // pop-up
			$input.wrap('<span class="calendar_wrap"></span>').after(
	...
	setupActions : function() {
		...
		$('.calendar_titleRow a').click( function() { // change first day of
					...
					popUpCal.showCalendar();
					popUpCal.setPos($(this.input).get(0), $('#calendar_div'));
				});
	...
	adjustDate : function(offset, period, dontShow) {
		...
		if (!dontShow) {
			this.showCalendar();
			this.setPos($(this.input).get(0), $('#calendar_div'));
		}
	},
	...
	setPos : function(targetObj, moveObj) {
		...
		var top;
		var left;
		var winSize = {w: document.body.clientWidth, h: document.body.clientHeight};
		var height = moveObj.get(0).scrollHeight;
		if ((coords[1] + targetObj.offsetHeight + moveObj.get(0).scrollHeight) < winSize.h) {
			top = coords[1] + targetObj.offsetHeight + "px";
		} else {
			top = coords[1] - 188 + "px";
		}
		if ((coords[0] + moveObj.get(0).scrollWidth) < winSize.w) {
			left = coords[0] + "px";
		} else {
			left = coords[0] + targetObj.offsetWidth - moveObj.get(0).scrollWidth + "px";
		}
		moveObj.css('height', height+"px");
		moveObj.css('position', 'absolute').css('left', left).css(
				'top', top);
	},
