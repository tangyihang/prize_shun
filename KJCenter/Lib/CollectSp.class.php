	...
	function __construct($url='')
   {
    ...
    public function savetoSPinfo($data){
       ...
       if($data['lotteryid']=='' || $data['lotttime']=='' || $data['ballid']=='' || $data['spinfo']=='' ){
		 ...
		 $sql="select spinfo,single,lasttime from lottery_spinfo where lotteryid='".$data['lotteryid']."' and lotttime='".date("Y-m-d",strtotime($data['lotttime']))."' ";
		 $sql.=" and ballid='".$data['ballid']."' and spinfo='".$data['spinfo']."' and single='".trim($data['single'])."' and lasttime='".trim($data['gameendtime'])."'";
		 ...
		 $num =$this->DB->execute($sql);
       	...
       	if(!$num){
            ...
            if($this->DB->data($arr)->table("lottery_spinfo")->add()){
               echo "success<br>";
            }else{
               echo "error<br>";
            }
