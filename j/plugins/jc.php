...
$money    = isset($_REQUEST['money']) ? intval($_REQUEST['money']) : '';//投注钱数
$multiple = isset($_REQUEST['multiple']) ? trim($_REQUEST['multiple']) : 0;//投注倍数
$childtype           = isset($_REQUEST['user_select'])?trim($_REQUEST['user_select']):'';//过关方式
$lotterycode    = isset($_REQUEST['combination']) ? trim($_REQUEST['combination']):'';//投注信息yuhua_lotterymode
$sport = isset($_REQUEST['sport']) ? trim($_REQUEST['sport']):'';//投注信息yuhua_lotterymode
...
$Opt=new optimize($lotterycode,$childtype,$type,$lotteryvalue,$sport);
$Opt->make();
$Opt->result();
